...
describe('POST /users/:userId/send-friend-request', () => {
  ...
  it('should return a 401 error if the user is not logged in', async () => {
    authenticatedSession.post('/api/auth/logout').expect(200);
    const res = await request(app)
      .post(`/api/users/${userToSendRequestTo._id}/send-friend-request`)
      .expect(401);
  });
  ...
  it('should return a 404 error if the user to send the friend request to is not found', async () => {
    const res = await request(app)
      .post('/api/users/invalid_id/send-friend-request')
      // .set('Authorization', `Bearer ${currentUser.generateAuthToken()}`)
      .expect(404);
